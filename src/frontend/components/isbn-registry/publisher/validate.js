...
export function validate(values) {
  ...
  integerStringFields.forEach(field => {
    if (values && values[field] && !regexPatterns.integerString.test(values[field])) {
      errors[field] = 'format.integer';
    }
  });
  ...
  if (values?.www && !regexPatterns.www.test(values.www)) {
    errors.www = 'format.www';
  }
