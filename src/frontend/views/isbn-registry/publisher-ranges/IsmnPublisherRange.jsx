...
function IsmnPublisherRange(props) {
  ...
  const {
    ...
    url: `/api/isbn-registry/publisher-ranges/ismn/${id}`,
    method: 'GET',
    authenticationToken,
    dependencies: [id],
    prefetch: true,
  ...
  async function handleDeleteRange() {
    ...
    await makeApiRequest({
      url: `/api/isbn-registry/publisher-ranges/ismn/${subRange.id}`,
      method: 'DELETE',
      authenticationToken,
      setSnackbarMessage,
      history,
  ...
  async function handleOpenRange() {
    const result = await makeApiRequest({
      url: `/api/isbn-registry/publisher-ranges/ismn/${subRange.id}/open`,
      method: 'POST',
      authenticationToken,
      setSnackbarMessage
    });
  ...
  async function handleCloseRange() {
    const result = await makeApiRequest({
      url: `/api/isbn-registry/publisher-ranges/ismn/${subRange.id}/close`,
      method: 'POST',
      authenticationToken,
      setSnackbarMessage
    });
  ...
  async function handleActivateRange() {
    const result = await makeApiRequest({
      url: `/api/isbn-registry/publisher-ranges/ismn/${subRange.id}/activate`,
      method: 'POST',
      authenticationToken,
      setSnackbarMessage
    });
  ...
  async function handleDeactivateRange() {
    const result = await makeApiRequest({
      url: `/api/isbn-registry/publisher-ranges/ismn/${subRange.id}/deactivate`,
      method: 'POST',
      authenticationToken,
      setSnackbarMessage
    });
  ...
  return (
    <Grid item xs={12} className="rangeContainer">
      <Typography variant="h5" className="subRangesTitleColorISMN">
...
export default withRouter(IsmnPublisherRange);
